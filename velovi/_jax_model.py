...
class JaxVELOVI(JaxTrainingMixin, BaseModelClass):
    ...
    gamma_init_data
        Whether to initialize the gamma parameter with the data.
    **model_kwargs
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        alpha_1_unconstr = (
            np.zeros(us_upper.shape).ravel()
            if not time_dep_transcription_rate
            else None
        )
    ...
    def get_rates(self):
        ...
        gamma, beta, alpha, alpha_1, lambda_alpha = self.module._get_rates_from_params()
        ...
        return jax.tree_util.tree_map(lambda x: self.module.as_numpy_array(x), rates)
